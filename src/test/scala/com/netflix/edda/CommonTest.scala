...
class CommonTest extends FunSuite {
  ...
  test("getProperty") {
    ...
    expectResult("default") {
      Common.getProperty("edda", "setting", "my.context", "default").get
    }
  ...
  "list" : [
    ...
    expectResult(expected) {
      Common.toPrettyJson(Map("foo" -> "bar", "list" -> List(1, 2, 3, 4), "date" -> date))
    }
  }
  ...
  test("writeJson") {
    ...
    expectResult(expected) {
      Common.toJson(Map("foo" -> "bar", "list" -> List(1, 2, 3, 4), "date" -> date))
    }
    expectResult(expected) {
      Common.toJson(Map("foo" -> "bar", "list" -> Seq(1, 2, 3, 4), "date" -> date))
    }
    expectResult(expected) {
      Common.toJson(Map("foo" -> "bar", "list" -> Range(1, 5), "date" -> date))
    }
  }
...
"""
    expectResult(expected) {
      Common.diffRecords(Seq(r2, r1), None, "collection/path")
    }
...
"""
    expectResult(expected) {
      Common.diffRecords(Seq(r5, r4, r3), None, "collection/path")
    }
...
"""
    expectResult(expected) {
      Common.diffRecords(Seq(r5, r4, r3), Some(0), "collection/path")
    }
  }
}
