...
abstract class Queryable extends Observable {
  ...
  /** query a collection for Records.
    *
    * @param queryMap query criteria to select records.  See [[BasicRecordMatcher]]
  ...
  def query(
    ...
    Common.namedActor(this + " query client") {
      ...
      Actor.self.reactWithin(queryTimeout) {
        case msg @ QueryResult(_, results) =>
          stopwatch.stop()
  ...
  private def localTransitions: Transition = {
    case (gotMsg @ Query(_, queryMap, limit, live, keys, replicaOk), state) =>
      ...
      scala.concurrent.future {
        ...
        logger.debug(s"$req$this sending: $msg -> $replyTo")
        replyTo ! msg
      } onFailure {
