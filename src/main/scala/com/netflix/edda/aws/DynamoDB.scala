...
object DynamoDB extends StrictLogging {

  def init(
    tableName: String,
    readCap: Long,
    writeCap: Long
  )(implicit client: AmazonDynamoDBClient) {
  ...
  def get(
    ...
    name: String,
    value: String
  )(implicit client: AmazonDynamoDBClient, req: RequestId): Option[Map[String, String]] = {

    import collection.JavaConverters._

    val getRequest = new GetItemRequest()
      .withTableName(tableName)
      .withKey(Map(name -> new AttributeValue(value)).asJava)
    ...
    val item = try {
      ...
      logger.info(s"$req dynamo read lapse: ${lapse}ms")
    }
        ...
        Option(attr.getS) orElse Option(attr.getN) orElse Option(attr.getB) orElse None match {
          ...
          case _ =>
            throw new java.lang.RuntimeException(
              s"key $key is none of [String,Number,Binary] on record $value in table $tableName"
            )
        }
      }.toMap
    )
  ...
  def toAttributeValue(value: Any): AttributeValue = {
    value match {
      ...
      case _: java.lang.Integer | _: java.lang.Long | _: java.lang.Float | _: java.lang.Double =>
        new AttributeValue().withN(value.toString)
      case v: Array[Byte] =>
        new AttributeValue().withB(java.nio.ByteBuffer.wrap(v))
      case _ =>
        throw new java.lang.RuntimeException(
          s"unable to convert ${value.getClass.getName} to DynamoDB AttributeValue"
        )
    }
  }
  ...
  def put(
    ...
    expected: Map[String, Any] = Map()
  )(implicit client: AmazonDynamoDBClient, req: RequestId): Any = {
    ...
    try {
      ...
      client.putItem(
        request
          .withTableName(tableName)
          .withItem(
            attributes.map(pair => pair._1 -> toAttributeValue(pair._2)).asJava
          )
          .withExpected(
            expected
              .map(pair => {
                ...
                pair._2 match {
                  ...
                  case _ =>
                    name -> new ExpectedAttributeValue()
                      .withExists(true)
                      .withValue(toAttributeValue(pair._2))
                }
              })
              .asJava
          )
      ...
      case e: InternalServerErrorException =>
        ...
        // with this exception we have no idea if the write actually happened, so lets fetch the
        // record and see if it matches what we just tried to write before rethrowing an exception
      ...
      val t1 = System.nanoTime()
      val lapse = (t1 - t0) / 1000000
      logger.info(s"$req dynamo write lapse: ${lapse}ms")
    }
  }
}
